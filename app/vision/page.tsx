  ...
  return (
    <main className="page">
      ...
      <div className="flex gap-2 mb-5 p-1 rounded-xl" style={{ background: '#141414' }}>
        {([
          { key: 'vision', icon: Star,   label: 'Vision Board' },
          { key: 'goals',  icon: Target, label: `Ziele${totalGoals > 0 ? ` ${doneGoals}/${totalGoals}` : ''}` },
          ...
          <div className="flex flex-col gap-3 mb-3">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div>
                ...
                <input placeholder="z.B. 10.000 Abonnenten auf YouTube"
                  value={vForm.title} onChange={e => setVForm({ ...vForm, title: e.target.value })} />
              </div>
              <div>
            ...
            <div>
              <label className="text-xs mb-1 block" style={{ color: '#888' }}>Motivation</label>
              <textarea placeholder="Warum ist dieses Ziel wichtig für euch?"
                value={vForm.description} onChange={e => setVForm({ ...vForm, description: e.target.value })} rows={2} />
            </div>
            <div>
              <label className="text-xs mb-1 block" style={{ color: '#888' }}>Deadline (optional)</label>
              <input type="date" value={vForm.deadline}
                onChange={e => setVForm({ ...vForm, deadline: e.target.value })} />
            </div>
            <div>
              <label className="text-xs mb-1 block" style={{ color: '#888' }}>Emoji</label>
              <div className="flex flex-wrap gap-2">
                {EMOJIS.map(em => (
                  <button key={em} className="text-xl rounded-lg p-1.5"
                    style={{ background: vForm.emoji === em ? '#2e2e2e' : 'transparent' }}
        ...
        <div className="card mb-5">
          ...
          <div className="flex flex-col gap-3 mb-3">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div>
                <label className="text-xs mb-1 block" style={{ color: '#888' }}>Titel</label>
                <input placeholder="z.B. 100 Videos hochladen"
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input type="number" inputMode="numeric" placeholder="z.B. 10000"
                  value={gForm.target_value} onChange={e => setGForm({ ...gForm, target_value: e.target.value })} />
              </div>
              <div>
                <label className="text-xs mb-1 block" style={{ color: '#888' }}>Deadline</label>
                <input type="date" value={gForm.deadline}
                  onChange={e => setGForm({ ...gForm, deadline: e.target.value })} />
              </div>
            </div>
            <div>
              <label className="text-xs mb-1 block" style={{ color: '#888' }}>Beschreibung</label>
              <input placeholder="Details zum Ziel"
                value={gForm.description} onChange={e => setGForm({ ...gForm, description: e.target.value })} />
            </div>
          </div>
          <div className="flex gap-2">
      ...
      {/* Vision Board — 1 col mobile, 2 col sm, 3 col desktop */}
      {tab === 'vision' && (
          ...
          {visions.length === 0 ? (
            ...
            <div key={v.id} className="card" style={{ position: 'relative' }}>
              <button className="btn p-1"
                style={{ position: 'absolute', top: 10, right: 10, color: '#444', borderColor: 'transparent' }}
              ...
              <h3 className="text-sm font-medium mt-2 mb-1 pr-5">{v.title}</h3>
              ...
              {v.deadline && (
                <p className="text-xs mt-2" style={{ color: '#555' }}>
                  Bis {format(parseISO(v.deadline), 'd. MMM yyyy', { locale: de })}
                </p>
              )}
            </div>
          ))}
        ...
        <div className="flex flex-col gap-3 mb-6">
          {goals.length === 0 ? (
            ...
            <div key={g.id} className="card flex items-start gap-3">
              <button onClick={() => toggleGoal(g.id, g.achieved)}
                className="flex-shrink-0 w-5 h-5 rounded border flex items-center justify-center mt-0.5"
                style={{
                  border: g.achieved ? '1px solid #4ecca3' : '1px solid #444',
                  background: g.achieved ? '#0a2e24' : 'transparent'
                }}>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1 flex-wrap">
                  <span className="text-sm font-medium"
                    style={{ textDecoration: g.achieved ? 'line-through' : 'none', color: g.achieved ? '#555' : 'white' }}>
                    {g.title}
                  </span>
                ...
                {g.deadline && (
                  <p className="text-xs mt-1" style={{ color: '#555' }}>
                    Deadline: {format(parseISO(g.deadline), 'd. MMMM yyyy', { locale: de })}
                  </p>
                )}
                {g.target_value && (
                  <p className="text-xs mt-1 font-semibold" style={{ color: '#888' }}>
                    Ziel: {g.target_value.toLocaleString('de-DE')}
                  </p>
                )}
              </div>
              <button className="btn p-1 flex-shrink-0"
                style={{ color: '#444', borderColor: 'transparent' }}
