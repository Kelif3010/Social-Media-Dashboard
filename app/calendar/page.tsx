  ...
  return (
    <main className="page">
      ...
      <div className="card mb-4">
            ...
            return (
              <div key={day.toISOString()} className="rounded-lg p-1 min-h-10"
                ...
                {dayEvents.map(e => {
                  ...
                  return (
                    <div key={e.id} className="rounded text-center mb-0.5"
                      style={{ background: et?.bg, height: 5, width: '100%' }} title={e.title} />
                  )
                })}
              </div>
            )
        ...
        <div className="flex gap-3 mt-3 pt-3 border-t" style={{ borderColor: '#1e1e1e' }}>
          {EVENT_TYPES.map(t => (
      ...
      <div className="card mb-6">
        <h3 className="text-xs font-medium mb-3 uppercase tracking-wider" style={{ color: '#666' }}>Kommende Termine</h3>
        {upcomingEvents.length === 0 ? (
          ...
          return (
            <div key={e.id} className="flex items-center justify-between py-2.5 border-b" style={{ borderColor: '#1e1e1e' }}>
              ...
              <button className="btn p-1.5" style={{ color: '#f87171', borderColor: 'transparent' }} onClick={() => deleteEvent(e.id)}>
                <Trash2 size={13} />
              </button>
            </div>
          )
