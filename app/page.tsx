...
import { supabase, CalendarEvent, Milestone } from '@/lib/supabase'
...
export default function Overview() {
  ...
  async function fetchAll() {
    ...
    const [{ data: subData }, { data: evData }, { data: msData }] = await Promise.all([
      supabase.from('subscriber_history').select('*').order('recorded_at', { ascending: true }),
          ...
          return (
            <div key={key} className="card">
              ...
              <div className="flex items-end justify-between">
                <div>
                  <div className="text-3xl font-semibold">{count.toLocaleString('de-DE')}</div>
                  <div className="text-xs mt-0.5" style={{ color: '#666' }}>Abonnenten / Follower</div>
                </div>
                {hist.length > 1 && (
                  <div className="text-right">
                    ...
                    <div className="text-xs" style={{ color: '#555' }}>letzte Änderung</div>
                  </div>
                )}
              </div>
              {goal > 0 && (
